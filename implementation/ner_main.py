...
if __name__ == '__main__':
	...
	# esp_tr_sents = corpus.read_conll_data(args.ESP_PATH_TR, encoding='latin-1', lang='esp') # **UNCOMMENT**

	# for ner_sys in [ner_system.AlwaysNonEntity, ner_system.SingleEntity, ner_system.FirstOrderHMM, ner_system.SecondOrderHMM]:
	# for ner_sys in [ner_system.AlwaysNonEntity]:
	# for ner_sys in [ner_system.SingleEntity]:
	# for ner_sys in [ner_system.FirstOrderHMM]:
	for ner_sys in [ner_system.MEMM]:
		# ner_sys1, ner_sys2 = ner_sys(tr_sents), ner_sys(esp_tr_sents) # HERE
		ner_sys = ner_sys(tr_sents)
		# for te_path in [args.ENG_PATH_VA, args.ENG_PATH_TE]:
		for te_path in [args.ENG_PATH_TE]:
